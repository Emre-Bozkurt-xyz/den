...
import { GalleryLimits, type GalleryAlbumsResponse, type GalleryResponse, type MediaKind, type TagsAutocompleteResponse } from '@den/shared';
...
export async function galleryRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Params: { id: string }; Querystring: { kind?: string; q?: string; before?: string; limit?: string } }>(
    ...
    async (req) => {
      ...
      if (kindRaw && !MEDIA_KINDS.has(kindRaw)) throw validation('kind must be image, video, or voice');
      const kind = (kindRaw as MediaKind | undefined) ?? null;
