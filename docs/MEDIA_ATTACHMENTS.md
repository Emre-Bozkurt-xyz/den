...
- **Three entry points, one funnel** (added 2026-09-01): the attach button, clipboard paste (docs/IMAGE_PASTE.md), and now the in-app camera (docs/CAMERA_COMPOSER.md) all land on `ChatView.handleAddFiles` → `stageFiles`. A capture is just a `File` like any other; nothing about the camera path is special-cased downstream, and no entry point carries its own validation.
