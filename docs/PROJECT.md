...
Current types: `hello ping pong error` · `message.send message.new` · `chat.created` · `friend.request friend.accepted` · `media.ready` · `tag.added tag.removed` · `message.deleted message.restored` · `reaction.added reaction.removed`.
...
- Real-device-verified so far (2026-07-21/22, prod): iOS push, iOS voice round-trip, install flow, Android PWA general pass. Still unverified on iOS: HEIC upload, cold-start session persistence, most touch-gesture feel.
