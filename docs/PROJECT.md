...
**In flight:** Embeds & Vault portal (`docs/EMBEDS.md`, pulled forward by owner decision, off the ordered roadmap below). Phases 1–2 shipped: embed framework + Instagram provider, Vault account linking (outbound OAuth client). **The entire Vault side (`the-vault/docs/DEN_EMBED_BRIDGE.md` §A/§B/§C) is implemented as of 2026-07-27**, so Phases 3–4 (Vault read-embeds, the Stage + live portal editing) are unblocked.
