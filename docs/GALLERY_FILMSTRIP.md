...
| F2 | **Scrolling the rail selects the centred slot** (revised 2026-08-12 after the owner used it: tap-only "does nothing but scroll the rail"). Committed on **settle**, not live. | Phone-gallery behaviour. Settle-not-live because the main view shows full-size media: selecting on every scroll frame would fetch a full-size image per slot crossed, *and* would re-render the gallery's un-virtualized grid behind the viewer on every frame. Note this is *scroll*-driven, not a pointer-drag: no custom gesture, so magnification can never displace what's under the finger (the original drag-seek hazard). Any future pointer-driven seek MUST still map finger-x to **fixed slot positions** with magnification paint-only. |
| F2c | **The main view swaps to the cached thumbnail immediately** (`onPreview`), while the full-size fetch still waits for settle. | The settle delay alone "feels quite bad" (owner) — the stage sat frozen mid-scrub. The rail has already painted every visible thumbnail, so they're in cache and cost nothing to show. This is reported straight from the rail to the viewer's own local state, so **nothing above the viewer re-renders during a scrub** — the grid stays untouched. The stand-in is layered *over* the real media and clears on its `load`/`loadeddata`, not on commit: clearing on commit would show thumb → blank → full-size, which is worse than the lag it replaced. A cached full-size image is already `complete` before React can attach `onLoad`, so an effect covers that case explicitly or the viewer would sit on a soft thumbnail forever. |
