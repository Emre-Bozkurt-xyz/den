...
- **The add flow (settled 2026-07-27).** Tapping the add-card opens a menu whose only category today is **Vault doc**; choosing it opens a **single picker modal** that carries both actions rather than an intermediate second menu (owner decision — with only two options the extra step doesn't earn its tap):
  - **First row: `+ New blank doc`** → `POST /api/embed/documents { title, groupId }` (service bearer) → insert `chat_vault_docs` → **open the portal immediately** (you made it to write in it).
  - **Below it: the clonable-doc list** → `GET /api/embed/documents?query=` with the *acting user's* bearer, rendered as title + smaller desaturated `folderPath` beneath. Picking one clones it into the chat's group (`{ sourceDocumentId, groupId }`, dual credential) → insert `chat_vault_docs` → **return to the Stage grid** (the content already exists; no reason to force an editor).
  - **Ownership is always "a copy owned by the group"** — never a reference to a member's personal doc, which would vanish or lose access when they leave. This is exactly why Vault's clone exists rather than an "adopt" op.
  - The list is **only available to linked users** (it needs their OAuth token); an unlinked member sees a "Connect Vault" prompt in place of it. Keep the menu component even though there's one category — a second category (pinned notes, other providers) then costs one entry, not a redesign.
  - **Precondition for both actions:** `ensureChatGroup(chatId)` then `ensureMember(chatId, userId)` must complete first — see §7.1 item 1. A missing mirror makes clone return an opaque 404.
...
- **The "paper thumbnail" (recommended, ports from Vault's own gallery).** Vault already solved the sparse-card problem with a Google-Docs-style fake page thumbnail, and the technique is **still live** in Vault today: `components/workspace/WorkspaceDocumentPreviewCard.tsx` + the `.vault-doc-preview*` rules in `app/styles/components.css`. It is **not a rendered-to-image thumbnail** — it renders real content into a "sheet" and fakes the paper look with CSS: a `4/3` preview box, `.vault-doc-preview-content { transform: scale(0.82); transform-origin: top left; width: 122%; }` (miniature of the top of the page), a hairline top edge, and a bottom gradient fade so the clipped text trails off like a real sheet.
  ...
  - ⚠️ **Private images will be missing** from both the thumbnail and the read view (§7.1 item 5) — the snapshot's private-asset URLs need a Vault session cookie Den's viewers don't have. Text, structure, and public assets render fine. Don't chase this from Den.
...
**B2. Clone-picker list (Phase 4)** — *added Vault-side 2026-07-27*
- `GET /api/embed/documents?query=&scope=owned|shared|all&limit=50` (**acting-user bearer**) → `{ documents: [{ id, title, folderPath, ownerName, visibility, updatedAt, snippet }] }`. All params optional; `limit` 1–100 (default 50), `query` ≤200 chars matches title+body, invalid → 400, rate limit 60/min → 429. Banned user → `200` with an empty list (never confirms existence).
- **Deliberately excludes group-owned docs** — the picker means "docs of *yours* you could bring into this chat." This makes the list **narrower than what clone accepts** (clone authorizes via `getDocumentAccess`, which honors group membership); that asymmetry is intentional, so never treat "absent from list" as "not clonable."
...
- `POST /api/embed/documents` **(dual credential)** `{ sourceDocumentId, groupId, title? }` → `{ documentId }` — **clone**. `Authorization: Bearer <service_token>` authorizes writing into the group; **`X-Vault-Acting-User-Token: <user_oauth_token>`** authorizes *reading the source*. Same user token Den already stores — no new grant or scope. Ordered checks: service token (401) → header present (400) → user token valid (401) → group exists & owned by this service (404) → user not banned (404) → **user is a member of `groupId`** (404) → user can read source (404). All 404s are opaque.
- **CSP:** Vault sets `frame-ancestors` for the whole `/embed/*` family centrally (`proxy.ts` + `lib/security/csp.ts`, env-configurable via `EMBED_FRAME_ANCESTORS`) — exactly Den's origin, never `*`.
- **Boot token:** short TTL (≤60s), single-use, bound to `{ documentId, vaultUserId }`, signed off `AUTH_SECRET`. A refresh of the iframe re-uses the token and therefore **fails closed** — Den must mint a fresh session per open, never reuse an `embedUrl`.

### 7.1 Vault-side realities Den must design around (from the 2026-07-27 bridge revisions)

1. **Membership must be mirrored *before* the first clone/create.** Clone check 6 requires the acting user to already be a `group_members` row, and the failure is a `404` indistinguishable from "no such group." So the Stage add-flow does `ensureChatGroup` + `ensureMember` **synchronously as a precondition**, not lazily via the sweep. The realistic race is a fresh chat's very first add.
2. **Service principal + service token.** Den is a `services` row seeded by Vault's `scripts/seed-service.mjs --slug den --name "Den"`, which prints a plaintext `service_token` **once**. Den needs it as `VAULT_SERVICE_TOKEN` (server-only). Two-tier trust: the OAuth client acts *as a user*; only the service token can create groups / own docs / manage membership.
3. **`ownerName` for a group doc is the group name**, not a person — the real `ownerId` is the `den-system` principal. Never surface "den-system" in Den's UI.
4. **Revocation is connect-time only, and that gap is real.** Removing a member takes effect when the collab client next connects; an already-open socket keeps editing until it reconnects. Don't imply instant revocation in UI copy.
5. **Private assets do not render in `/rendered`.** That endpoint's `<img>` URLs authorize by Vault session cookie, which Den's cross-origin viewers don't have; the embed-session-token workaround only applies inside the live editor iframe. **Consequence: Den's read view and the paper thumbnail show public assets only** — private images will be missing. This is a known Vault-side limitation, out of scope to fix from Den. Calendar blocks and sticker overlays are also omitted from the static render.
6. **Clone copies `document_assets` grants**, so it exposes the source author's private assets to every group member. Vault enforces the boundary but builds no warning UI — a confirmation is Den's call and is **deliberately skipped for v1** (owner decision 2026-07-27).
7. **Vault's group ownership had not been exercised against a real DB or a real client** when Den started integrating — Den is the first consumer, so treat first-contact failures as likely-Vault-side and verify against the live instance rather than assuming Den is at fault.
